...
class ApmsServer:
    def __init__(self, config_file=None):
        config.load_config(
            config_file if config_file is not None else config.default_config_file()
        )
        ...
        other_urls = [
            ...
            (r"/files/photos/(.*)", StaticFileHandler, {"path": config.photos_dir}),
            (r"/", MainHandler),
            (r"/photos", MainHandler),
            (r"/people", MainHandler),
            (r"/api-docs", MainHandler),
            (r"/(.*)", StaticFileHandler, {"path": config.static_dir}),
        ]
        ...
        self._app = Application(
            api_urls + other_urls, session_factory=self._session_factory
        ).listen(7777, "127.0.0.1")

    def run(self):
        tornado.ioloop.IOLoop.current().spawn_callback(
            self._updater.update_photos_of_next_user
        )
