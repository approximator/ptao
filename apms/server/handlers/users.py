...
async def get_user_from_remote(user_id):
    ...
    info["url"] = (
        f'{config.raw_data["api_clients"].get("host_url_base", "")}/{info["domain"]}'
        if kind == 0
        else info["screen_name"]
    )
    ...
    info["status_str"] = (
        info.get("status", "") if kind == 0 else info.get("description", "")
    )

    usr_fields = [
        fld_name
        for fld_name, fld_type in User.__dict__.items()
        if str(fld_type).startswith("User")
    ]
