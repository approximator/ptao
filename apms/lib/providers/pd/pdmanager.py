...
class PDManager:
    ...
    async def get_user_info(self, user_id):
        resp = await self._api.users.get(
            user_ids=user_id, fields=self._info_fields, name_case="Nom"
        )
        log.debug(json.dumps(resp, indent=4))
        return resp
    ...
    async def get_group_info(self, group_id):
        resp = await self._api.groups.getById(
            group_id=group_id, fields=self._group_info_fields
        )
        log.debug(json.dumps(resp, indent=4))
        return resp
    ...
    async def get_all_photos(self, user_id, albums_id=None):
        ...
        if albums_id is not None:
            ...
            for album_id in albums_id.split(","):
                ...
                photos.extend(
                    await PDManager._get_all_items(self._api.photos.get, params)
                )
            return photos
